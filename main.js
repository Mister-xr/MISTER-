...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [MISTER MD] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'MISTER MD' };
            }
        });
    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    ...
                    const welcomeBotName = welcomeConfig.BOT_NAME || 'MISTER MD';
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const displayBotName = effectiveConfig.BOT_NAME || 'MISTER MD';
                const myquoted = {
        ...
        arslanLog(`MISTER MD-MINI Pair error: ${err.message}`, 'error');
